...
Remaining: settings pages, theme editor, persisted dock layout, detached response windows, richer tab actions such as pin/reorder/reopen, complete focus and accessibility audit, and real Wayland/X11 visual smoke testing on a supported desktop host.
...
Remaining: native collection and folder mutation dialogs, drag-and-drop wiring, advanced tab actions and persistence, inheritance inspector UI, variable autocomplete and navigation, OS keyring or encrypted vault integration, polling fallback for unreliable filesystems, more importer/exporter formats, and native search, import, and code-generation panels.
...
Remaining: deeper native GUI integration, broader compatibility coverage, and release-host validation for long-running scheduled workflows.
...
Remaining: production GraphQL subscription transport, production WebSocket/SSE adapters, production gRPC networking and reflection transport, and experimental HTTP/3, Socket.IO, MQTT, raw TCP, and Unix-socket HTTP modules.
...
Remaining: Swagger 2 import, automatic remote reference retrieval, deeper schema coverage, richer native OpenAPI workflows, and integrated mock-server TLS termination.
...
Remaining: full native Git workflows, a bundled WebAssembly executor, broader extension APIs, and polished native AI workflows.
...
Remaining release-environment gates: native GPUI desktop smoke testing, screen-reader and keyboard-only audit, signed and notarized macOS packages, signed Windows installers, clean Arch/Debian/RPM/Nix/AppImage builds, long-running fuzz campaigns, and final dependency-audit review for the release commit.

...
ApexAPI checkpoint status was verified on 2026-06-29.
...
| Native non-Web UI | No | No | No | No | No | **Implemented shell**: GPUI binary links and native panels; desktop GPU smoke pending |
...
| Multi-document tab lifecycle | Supported | Supported | Supported | Supported | Supported | **Implemented**: preview/pin/close/reopen/reorder/overflow/session restoration and keyboard navigation |
...
| WebSocket/SSE/gRPC | Broad protocol tooling | Focused HTTP/GraphQL | Strong real-time breadth | Multiple protocols | Evolving | **Native foundation UI**: GraphQL validation/build; bounded WebSocket/SSE logs; gRPC validation/reflection request generation; live streaming adapters remain external |
| Scripts/assertions | Mature | Limited | Scripts/tests | Scripts/tests | Scripts/tests | **Native automation UI**: real HTTP collection runs, retries, cancellation, and JSON/JUnit/HTML reports; constrained scripts/assertions remain backend-owned |
| OpenAPI lifecycle | Broad | Import | Import/generation | Design/spec workflows | Import | **Native foundation UI**: OpenAPI operation browsing, Markdown, guarded request generation |
...
| Local mock server | Supported | Varies | Supported | Supported | Supported | **Implemented native control**: real loopback server start/log/stop; TLS remains adapter-owned |
| Constrained third-party plugins | Product-specific | Product-specific | Product-specific | Plugin ecosystem | Extension model | **Native validation UI**: explicit capability approvals and bounded module validation; bundled runtime remains unavailable |
...
| Collection/folder/request mutations | Implemented native foundation | Guarded create/rename/move/duplicate/archive/delete and ordering controls |
| Incremental global request search | Implemented native foundation | Rebuildable bounded SQLite index, sensitive values excluded, result navigation |
...
| Tab lifecycle | Implemented foundation | Visual open/activate/close, preview promotion, dirty and pin guards, reopen-closed, close-other/right, left/right reorder controls, bounded overflow navigation, startup restoration, active-resource preservation, history draft tabs; drag reorder pending |
...
| Git UI/WASM plugins/AI | Implemented native foundation | Git commands, bounded capability-approved plugin validation, redacted AI preview/confirmation; no bundled WASM runtime or remote AI provider |
